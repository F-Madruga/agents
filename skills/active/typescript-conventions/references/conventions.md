...
Declare a single destructured object parameter instead of positional arguments, so every value is named at the call site and order stops mattering.

```ts
// Don't. Two strings; swap them and it still compiles.
function createUser(name: string, email: string) {}

// Do. Named by the caller, order-independent.
function createUser({ name, email }: { name: string; email: string }) {}
```

The same holds for the arguments you pass on:
...
Treat arguments as read-only. A caller who hands you a value shouldn't find it changed underneath them; produce a new value and return it.

```ts
// Don't. The caller's value mutates behind their back.
function addItem({ items, item }: { items: Item[]; item: Item }): void {
  items.push(item);
}

function activate({ agent }: { agent: Agent }): void {
  agent.status = "active";
}

// Do. The input is untouched and the result is the return value.
function addItem({ items, item }: { items: Item[]; item: Item }): Item[] {
  return [...items, item];
}

function activate({ agent }: { agent: Agent }): Agent {
